...
cat /kafka/config/server.properties.template | sed \
  ...
  -e "s|{{ZOOKEEPER_CONNECTION_TIMEOUT_MS}}|${ZOOKEEPER_CONNECTION_TIMEOUT_MS:-300000}|g" \
  -e "s|{{ZOOKEEPER_SESSION_TIMEOUT_MS}}|${ZOOKEEPER_SESSION_TIMEOUT_MS:-300000}|g" \
  -e "s|{{GROUP_MAX_SESSION_TIMEOUT_MS}}|${GROUP_MAX_SESSION_TIMEOUT_MS:-360000}|g" \
  -e "s|{{REQUEST_TIMEOUT_MS}}|${REQUEST_TIMEOUT_MS:-60000}|g" \
